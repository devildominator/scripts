...
BITBUCKET_REVIEWERS="https://api.bitbucket.org/2.0/repositories/$BBPR_ID/$BBPR_REPO/default-reviewers"  #  -- reviewers work
##############################--PATCHING --- ############################################
...
$(curl -sS -H "Content-Type: application/json" -u $BBPR_EMAIL:$BBPR_PASS  $BITBUCKET_REVIEWERS > $BRANCH_FILE)
SIZE=$(grep -Po '"size": \d+' $BRANCH_FILE | sed 's/"size": \(\)/\1/g')
PAGELEN=$(grep -Po '"pagelen": \d+' $BRANCH_FILE | sed 's/"pagelen": \(\)/\1/g')
echo "reviewrs Total is $SIZE"
echo "reviewrs Max Per Page is $PAGELEN"
LOOPER=$(echo `/usr/bin/perl -w -e "use POSIX; print ceil($SIZE/$PAGELEN), qq{\n}"`)
if [ -z "$LOOPER" ] || [ $LOOPER -eq 0 ];
then
      echo "NO BRANCH RESULT FOUND IN [ $BBPR_REPO ] REPOSITORY.SIZE= $SIZE PAGELEN = $PAGELEN"
...
echo "REVIEWERS LOOPING UPTO $LOOPER"
$(truncate -s 0 $BRANCH_FILE)
for ((i=1; i<=$LOOPER; i++))
do
   echo $i
   echo $BITBUCKET_BRANCHS"?page=$i"
   $(curl -sS -H "Content-Type: application/json" -u $BBPR_EMAIL:$BBPR_PASS  $BITBUCKET_REVIEWERS"?page=$i" | grep -P '"username": "[\w\/.\-]+"' -o  |sed "s/\(.*\)/{\1},/" >>  $BRANCH_FILE)
done
if [ -s $BRANCH_FILE ]
then
	BBPR_REVIWERS=`tr '\n' ' ' < $BRANCH_FILE |sed 's/[\,[:blank:]]*$//' | tr -d ' '`
	BBPR_REVIWERS=',"reviewers":['$BBPR_REVIWERS']'
else
	echo "NO DEFAULT REVIEWERS FOUND"
        BBPR_REVIWERS=""
fi
echo $BBPR_REVIWERS
# -------------------   //

PULLOUT=`curl -sS -X POST -H "Content-Type: application/json" -u $BBPR_EMAIL:$BBPR_PASS -d '{"destination":{"branch":{"name":"'$NEXTBRANCH'"}},"source":{"branch":{"name":"'$CURRBRANCH'"}},"title":"JENKINS-PR '$BUILD_NUMBER'","description": "This is an automated pull request generated by Jenkins." '$BBPR_REVIWERS' }' $BITBUCKET_PULLREQUEST`
echo $PULLOUT
